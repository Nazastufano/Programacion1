...
begin
    ...
    begin
        suma:=suma+mat[i,j];
        if i>1 then
            AnalisisRecursivo:=AnalisisRecursivo(mat,N,M,i-1,j,x,suma)
        else
            if suma>=x then
...
begin
    ...
    readln(x);
    WriteLn('La cantidad de sucursales que tienen al menos ', x, ' empleados es: ', AnalisisRecursivo(mat,N,M,N,M,x,0));
end.