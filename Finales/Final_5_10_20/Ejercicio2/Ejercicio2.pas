...
begin
    If i>0 Then
    Begin
        If (mat[i,i]>mat[i,N-i+1]) Then
            val := 1
        Else
            val := 0;
        analisisRecursivo := val + analisisRecursivo(mat,N,i-1,i-1);
    End
    else
        analisisRecursivo := 0;
end;
...
begin
    cargarMatriz(mat,N);
    WriteLn('La cantidad de filas en que la diagonal principal supera a la secunadria es: ', analisisRecursivo(mat,N,N,N));
end.